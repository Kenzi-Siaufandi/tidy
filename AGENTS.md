...
- Git: non-empty workdir (e.g. Pterodactyl `/home/container`) uses `cloneInPlace` (init + fetch + checkout), not `git clone`. `Pull` runs `reset --hard HEAD` first to discard templated changes. Before pulling, drift versus the remote is snapshotted into `.tidy/drift-YYYY-MM-DD_HH-MM-SS.txt` (one file per drift event, never pruned; nothing written on clean runs; `-N` suffix on same-second collisions; local time header; sections "Discarded on sync" = tracked mods minus last run's templated files, "Kept locally" = untracked, plus a capped unified `git diff HEAD` per tracked file — first 10 files, 80 lines each — showing exactly what the sync discarded) with a console warning; `.tidy/` paths are always filtered as self-pollution, volatile Minecraft date comments (`#Sun Sep 20 ...`, rewritten by every server boot, vanilla not Pterodactyl) never count as drift, and `DefaultGitignore` ships `.tidy/` so fresh repos stay clean. Panel edits would otherwise vanish silently. Auth via `GIT_ASKPASS` temp script — never embed tokens in URL/config. Env aliases: `GIT_REPO|GIT_URL|TIDY_GIT_REPO`, `GIT_BRANCH|TIDY_GIT_BRANCH` (default `main`), `GIT_TOKEN|GIT_AUTH_TOKEN|GITHUB_TOKEN`, `GIT_USER`. Flags (`--git-repo`, `--skip-git`, `--skip-templates`, `--config`, `--workdir`) override env.
- Console output: progress lines (`[*] Resolving/Checking/Downloading...`) are NOT printed — only the final result per artifact. Colors: green = steady/unchanged (up-to-date, skipped, "repository is up to date"), yellow = first install or any update/download/removal, red = fatal errors only, cyan = neutral progress/info. No doctor block; no "State: Saved" line (`state.SaveState` still runs silently).
- Idempotency: server/plugins skip download only if local file exists **and** SHA-256 verifies against state; stale server jar deleted only after new download succeeds. Removed plugins (in state, absent from config) are deleted from `plugins/`. Version upgrades swap only the jar and log a warning — plugin configs/data are left untouched. Interrupted downloads resume via HTTP `Range` from `<target>.tidy-tmp` (kept on stream errors, discarded on hash mismatch or unsatisfiable range).
