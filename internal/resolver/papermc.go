...
func (c *PaperClient) ResolveAndDownload(ctx context.Context, serverCfg config.ServerConfig, workDir string) (*ServerDownloadResult, error) {
	...
	endpoint := fmt.Sprintf("%s/v3/projects/%s/versions/%s/builds/%s",
		c.BaseURL, serverCfg.Project, serverCfg.Version, build)

	req, err := http.NewRequestWithContext(ctx, http.MethodGet, endpoint, nil)
	if err != nil {
		return nil, fmt.Errorf("failed to create PaperMC API request: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("failed to contact PaperMC Fill API (%s): %w", endpoint, err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("PaperMC Fill API returned HTTP %d %s for project %q version %q build %q",
	...
	if err := json.NewDecoder(resp.Body).Decode(&buildResp); err != nil {
		return nil, fmt.Errorf("failed to parse PaperMC Fill API response: %w", err)
	}

	// Select server download artifact
	download, ok := buildResp.Downloads["server:default"]
	if !ok {
		// Fallback: pick the first available entry in downloads
		for _, dl := range buildResp.Downloads {
			download = dl
			ok = true
			break
		}
	...
	if !ok || download.URL == "" {
		return nil, fmt.Errorf("no server download URL found in PaperMC Fill API response for %s %s build %d",
			serverCfg.Project, serverCfg.Version, buildResp.ID)
	}

	sha256Hash := strings.TrimSpace(download.Checksums.SHA256)
	if sha256Hash == "" {
		return nil, fmt.Errorf("no sha256 checksum provided by PaperMC Fill API for build %d", buildResp.ID)
	}

	filename := download.Name
	if strings.TrimSpace(filename) == "" {
		filename = fmt.Sprintf("%s-%s-%d.jar", serverCfg.Project, serverCfg.Version, buildResp.ID)
	}
